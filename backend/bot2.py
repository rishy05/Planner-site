...
async def add(ctx):
    ...
    repl = await client.wait_for("message")
    print(repl.content)
    if repl.content == "yes":
...
@client.command()
async def test(ctx):
    await ctx.channel.send("Send Yes or No")
    repl = await client.wait_for("message")
    print(repl.content)
    if repl.content == "yes":
        await ctx.channel.send("Greate you clicked YESSSSSSSSS")
    else:
        await ctx.channel.send("Greate you clicked NOOOOOOOOOO")
